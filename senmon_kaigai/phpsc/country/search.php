...
class GetLowPrice {
	...
	function __construct() {
		global $GlobalSolrReqParamAry,$naigai;

		$this->MyNaigai = $naigai;

		$saiyasune_type = 'tour';
		if(!empty($_REQUEST['p_bunrui'])){
			$saiyasune_type = 'freeplan';
		}

		$mamchache = new getSaiyasuneTourMemcache($saiyasune_type);
		// メモキャッシュから取得
		$this->returnObj = $mamchache->getMemCacheData();
...
//				$this->FacetPlane = $SolrObj->Obj->facet_counts->facet_fields;
			...
			// メモキャッシュに保存する
			new setSaiyasuneTourMemcache($this->returnObj,$saiyasune_type);
		}
