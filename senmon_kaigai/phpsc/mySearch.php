...
class LoadAction {
	...
	function __construct() {
		...
		$this->FacetPlane = (isset($SolrObj->Obj->facet_counts->facet_fields)) ? $SolrObj->Obj->facet_counts->facet_fields : null;
...
//		$this->dispObj['pager'] = $this->pager($SolrObj->Obj->response->p_hit_num, $this->reqPara['p_start_line']);
	...
	function ActErr($SolrObj){
		if($SolrObj->errFlg == 0){	//エラーじゃなかったら
			/*ファセットを使いやすくします*/
			$facetObj = (isset($SolrObj->Obj->facet_counts->facet_fields)) ? $SolrObj->Obj->facet_counts->facet_fields : null;
			$this->FacetObj = new SetFacet($facetObj);
			/*商品部分はそのまま使います*/
		...
		//エラー時の処理
			...
			/*エラーの内容を入れます*/
			$this->ErrObj = (isset($SolrObj->Obj->response->p_result_detail)) ? $SolrObj->Obj->response->p_result_detail : null;
		}
	}
