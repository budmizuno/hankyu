...
class getCsvItemClass
{
    ...
    function getCsv($filePath, $p_category = null)
    {
        ...
        if($filePath == MASTER_GUIDE_CSV_URL){
            // memcacheからデータ取得
            $csvData = $SenmonMemCache->getGuideCsvData(serialize('master_senmon_kaigai_guide_2017'));
            // memcacheにあるなら
            if ($csvData != false && !empty($csvData)) {
                ...
                // memcacheに設定
                $SenmonMemCache->setGuideCsvData(serialize('master_senmon_kaigai_guide_2017'), serialize($csvData));
            }
    ...
    public function getMasterCsvAllData()
    {
        ...
        // memcacheから取得
        $this->masterCsvAllData = $SenmonMemCache->getMasterCsvData(serialize('master_senmon_kaigai_2017'));
        if ($this->masterCsvAllData != false && !empty($this->masterCsvAllData)) {
            ...
            $SenmonMemCache->setMasterCsvData(serialize('master_senmon_kaigai_2017'), serialize($this->masterCsvAllData));
        }
    }
