...
class GetimportantBlogList {
	...
	public function blogToHtmlNew($knd, $naigai = '', $num = '', $width = '', $DateType = 0, $InfoLink = 0) {
		...
		if (strpos($knd, '重要なお知らせ') !== false) {
			...
			$Blog = $this->RetTagNewSenmon($XmlUrl, 'dd', $width, $num, $DateType,0);
...
EOD;
	...
	public function blogToNews($naigai = '', $num = '', $width = '', $DateType = 0, $InfoLink = 0) {
		...
		$Blog = $this->RetTagNewSenmon($XmlUrl, 'li', $width, $num, $DateType,1);
		...
		$html =<<< EOD
			<section class="yellow newsReleaseWrapper">
			  <h2>ニュースリリース</h2>
			  <ul class="blue adWrapper">
			  	{$Blog}
			  </ul>
			  <p class="btmLink"><a href="http://blog.hankyu-travel.com/newsrelease/" target="_blank"><span>ニュースリリースをもっと見る</span></a></p>
			</section>
EOD;
...
EOD;
	...
	#	ブログ記事リンク生成_リニューアル版
	##################################################
	private function RetTagNewSenmon ($URL = NULL, $TagName = 'li', $Width = '', $Row = '', $DateType = 0,$news_flag) {
		// 呼び出し
		...
		if($categoryType == CATEGORY_TYPE_DEST){
			...
			$kaigai_country = $array[KEY_MASTER_CSV_NAME_JA];
		}
...
EOD;
						...
						$Html .= <<< EOD
	<{$TagName}><a href="{$Obj->link}"><span>{$Title}</span></a></{$TagName}>
EOD;
					}
				}
				// 更新日あり
...
class blogDisp{
	...
	function GenchiSenmon(){
		...
		$XmlUrl = 'https://tabicoffret.com/rss/overseas' . $blogGenchi_url;
