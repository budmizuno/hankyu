$(function() {
    ...
    // banner
    /*$('#banner').bxSlider({ controls: false, auto: true });*/
    /*$('.sliderbx').bxSlider({
        minSlides: 3,
        maxSlides: 4,
        slideWidth: 300,
    ...
    $('.js-carousel-container').bxSlider({
        minSlides: 3,
        maxSlides: 4,
        slideWidth: 144,
        slideMargin: 14,
    ...
    */
    ...
    // 売れ筋ランキングの画像の横の長さを調整
    $(".wr-block .frame .uresuzi li").css('width',screenWidth * 0.7);

    // 担当者おすすめの画像の横の長さを調整
    $(".wr-block .frame .tantoshaOsusume li").css('width',screenWidth * 0.4);
...
function a() {
    $(".ttlCursSrchTxt").attr('placeholder', '例）E509');
}
...
$(function() {
    ...
    if($(".crystal-grey-list").find('li').length <= 0){
        $(".wrapper-crystal").eq(0).hide();
    }
    // フレンドツアー
    if($(".wrapper-crystal:eq(1) ul").find('li').length <= 0){
        $(".wrapper-crystal").eq(1).hide();
    }
