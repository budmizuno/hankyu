	...
	public function blogToNews($naigai = '', $num = '', $width = '', $DateType = 0, $InfoLink = 0) {
		...
		$html =<<< EOD
			<div class="idx_box03 news mb60">
			  ...
			  <p class="btn-more btn_newsrelease"><a href="http://blog.hankyu-travel.com/newsrelease/"><span>ニュースリリースをもっと見る</span> <span class="icon icon-arr-left"></span></a></p>
			</div>
EOD;
...
EOD;
				...
				elseif($this->naigai == 'd'){
				$li .=<<< EOD
<li><a href="{$data['link']}">{$data['time']}&nbsp{$data['ttl']}</a></li>
EOD;
				}
			}
			$this->Blog= $li;
