...
//出発地　段落ち
foreach($p_hatsuAry->TgDataAry['i'] as $key => $arr){
	foreach($arr as $kyotenCode => $val){
		$p_hatsuAry->TgDataAry['i'][$key][$kyotenCode] = '　'.$val;
	}
}

//p_hatsuの生成
$p_hatsu = (isset($p_hatsuAry->TgDataAry[$naigai][$kyotenId])) ? bindingHatsuAry($p_hatsuAry->TgDataAry[$naigai][$kyotenId]) : '';
...
//全国はcontentsブログを読む
if (strpos($KyotenID, 'top') !== false) {
    $KyotenID = str_replace('top', 'contents', $KyotenID);
}


if ($naigai == 'd') {
    $searchBoxtemp = $PathSharing14 . 'inc/Searchbox_d.php';
...
$p_bunrui = '030';
$p_hatsu_freeplan = '';
$p_hatsu_freeplan = (isset($p_hatsuAry->TgDataAry[$naigai][$KyotenID])) ? bindingHatsuAry($p_hatsuAry->TgDataAry[$naigai][$KyotenID]) : '';
// 検索のデフォルト発地
switch ($KyotenID) {
    case 'osa':
        $def_p_hatsu = bindingHatsuAry($p_hatsuAry->TgDataAry[$naigai]['osa']);
        break;
    default:
        $def_p_hatsu = $p_hatsu_freeplan;
        break;
}
