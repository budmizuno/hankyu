                    ...
                    <?php if($value['q_group'] == TOUR_URESUZI_RANKING):?>
                        ...
                        <li class="pos-rel swiper-slide">
                            <a href="<?=$value['tour_url'];?>">
                                ...
                                <?php endif; ?>
                                <p class="sly3-ct"><?php if($kyotenId == 'index') echo '<span>'.$value['p_hatsu_name'].'</span>'; ?><?=$value['p_course_name'];?></p>
...
<?php elseif((!isset($osusumeCsv[OSUSUME_CATEGORY_NUM][TOUR_URESUZI_RANKING]) || 0 == $osusumeCsv[OSUSUME_CATEGORY_NUM][TOUR_URESUZI_RANKING]) && !in_array($kyotenId, $uresuzi_ac_off_kyoten)): ?>
    ...
    <!-- レコメンド表示場所 -->
    <div id="ppz_recommend_spkaigaisenmon03"></div>
    <!-- レコメンド表示場所ここまで -->
    ...
    <script type="text/javascript">
        ...
        var ppz_spkaigaisenmon03 = new _PPZ();
        ppz_spkaigaisenmon03.cid = 21203;
        ppz_spkaigaisenmon03.rid = 15;
        ppz_spkaigaisenmon03.customer_id = '<?php e($customerId); ?>'; //顧客IDを代入

        ppz_spkaigaisenmon03.v02 = '1';//内外区分：海外
        ppz_spkaigaisenmon03.v04 = '<?php e($masterCsv[KEY_MASTER_CSV_DEST]); ?>';//目的地フラグを確認
        ppz_spkaigaisenmon03.v08 = "<?php e($p_hatsu);?>"; // ↑↑myHatsuに記載している出発地フラグをカンマ区切りにて記載して下さい。
        ppz_spkaigaisenmon03.v11 = '<?php e($masterCsv[KEY_MASTER_CSV_COUNTRY_LARGE]); ?>';//国コードを記載

        ppz_spkaigaisenmon03.rows = 10;                               //表示したいMAX件数を設定(最大20件)
        ppz_spkaigaisenmon03.cb = 'ppz_spkaigaisenmon03_ranking';               //ここの関数でレコメンド表示を作成してください。(ppz_draw.js)
        ppz_spkaigaisenmon03.div_id = 'ppz_recommend_spkaigaisenmon03';      //レコメンド表示を行うタグのIDを指定(divのIDと一致させます)
        ppz_spkaigaisenmon03.alt_html = '';
        ppz_spkaigaisenmon03.request();
    </script>
