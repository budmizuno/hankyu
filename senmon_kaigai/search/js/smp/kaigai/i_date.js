...
(function() {
	...
	this.CalendarService = (function() {
	  ...
	  CalendarService.createThisMonthYyyymm = function() {
	    ...
	    date = new Date(); // 現在の日付
	    date.setDate(date.getDate() + 7);	// 海外は現在の日付+7とする
...
//			calendarDisp(year+month,json);
	...
	CommonCalendarService.calendarDisp= function(tgDate,json){
//	var calendarDisp = function(tgDate,json){
		var daysInMonth = function (year,month){
			return new Date(year, month, 0).getDate();
		}

		tgDate.match(/([0-9]{4})([0-9]{2})/);
		var year = RegExp.$1
		var month = RegExp.$2;
		var zero_month = RegExp.$2;
...
//		initPagerMonthButtons();

//		createThisMonthYyyymm();

//		$(".month.calPagerMonth:eq(0)").toggleClass("selected");
//		$(".month.calPagerMonth:eq(0) a").attr('id',"pager_"+year+zero_month);
//		$(".month.calPagerMonth:eq(0) a").text(month+'月');


		var in_html = '';

		in_html += '<table class="">';
		in_html += '<caption class="">2016年6月（<a href="#">6月をすべて選択</a>）</caption>';
//				in_html += '<tr><td colspan="7" class="SW_SD_Caption" data-date="'+year+("0"+month).slice(-2)+'">'+year+'年'+month+'月</td></tr>';
		in_html += '<tr><th class="sun">日</th><th>月</th><th>火</th><th>水</th><th>木</th><th>金</th><th class="sat">土</th></tr>	';
		in_html += '<tr>';

		var z = 0
		for(i=0;i<week;i++){
			if(z%7 == 0 || z == 0){
				in_html += '<td class="sun">'+'</td>';
			}else if(z%7 == 6 || z == 6){
				in_html += '<td class="sat">'+'</td>';
			}else{
				in_html += '<td>'+'</td>';
			}
			z++;
		...
		for(i=0;i<lastDay;i++){
			...
			var tempI = ("0" + i).slice(-2);
			var ddate = tgDate+day;

			if(json != null && json.p_dep_day != null && json.p_dep_day[ddate]){

				if( z%7 == 0 || z == 0 || (json.holiday != null && $.inArray(ddate,json.holiday) >=0) ) {
					if(json.p_dep_day[ddate].facet > 0){
						in_html += '<td class="sun"><a href="javascript:void(0);" data-date="'+ ddate +'">'+(i+1)+'</a></td>';
					}else{
						in_html += '<td class="sun">'+(i+1)+'</td>';
					}
				}else if(z%7 == 6 || z == 6){
					if(json.p_dep_day[ddate].facet > 0){
						in_html += '<td class="sat"><a href="javascript:void(0);"  data-date="'+ ddate +'">'+(i+1)+'</a></td>';
					}else{
						in_html += '<td class="sat">'+(i+1)+'</td>';
					}
				}else{
					in_html += '<td><a href="javascript:void(0);"  data-date="'+ ddate +'">'+(i+1)+'</a></td>';
				}
			}else{
				if(z%7 == 0 || z == 0 || (json.holiday != null && $.inArray(ddate,json.holiday) >=0)){
					in_html += '<td class="sun">'+(i+1)+'</td>';
...
//		in_html += '</ul>';
		//		$('#search_date_page .tableBox').empty();
		//		$('#search_date_page .tableBox').html(in_html);

//		$("#modal_date .date-cal").empty();
		$(FormID+"#modal_date .cal-select").html(in_html);


		$(FormID+"a[data-date='"+$("#p_dep_date").val()+"']").css("background-color","#c8e68c");
		//▼日付選択
		$(FormID+"#search_date_page").on('click','.tableBox a', function(e){
			var date = $(this).attr("data-date");
			$(FormID+"#p_dep_date").val(date);

			$(FormID+"#search_date_page .tableBox a").each(function(index, element) {
				$(this).css("background-color","");
			});
			$(this).css("background-color","#c8e68c");
			//件数
			getHitNum('date');
		});
