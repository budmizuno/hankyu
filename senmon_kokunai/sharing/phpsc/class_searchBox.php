...
//echo "<pre>"; var_dump($KyoteHatsuAry); echo "</pre>";
//		echo $Naigai;
...
EOD;
					foreach($this->FacetObj->RetFacet[$CheckParamNameAry[$this->MyNaigai][1]][$DestCode] as $MyCode => $MyAry){
						...
						$selected = '';
						if(isset($mokuteki) && substr($mokuteki, 3, 2) == $MyCode){
							$selected = 'selected';
						}
...
EOD;
					...
					if(empty($CityAry)){
						//国ファセットから取得
						foreach((array)$this->FacetObj->RetFacet[$CheckParamNameAry[$this->MyNaigai][2]][$DestCode][$CountryCode] as $MyCode => $MyAry){
							if($MyAry['facet'] < 1){
								continue;
							}
							...
							if($categoryType == CATEGORY_TYPE_CITY){
								if(isset($mokuteki) && substr($mokuteki, 6, 5) == $MyCode){
									$selected = 'selected';
								}
							}
...
EOD;
					...
					if(!empty($CityAry) && count($CityAry) > 1){
	...
	function __construct(){
		...
		//方面はひとつ
			...
			if(count($CountrySplit) > 1){	//複数国
				...
				//HungLV_20150526_start #434修正
				if(($Request['fksCity'] && strlen($Request['fksCity']) > 0 && !$Request['preCity'] )
					|| ($Request['preCity'] && strlen($Request['preCity']) <= 0)){
					$cnt = 0;
					$citySplit = explode(',', $Request['fksCity']);
					foreach ($citySplit as $val) {
						if($cnt > 0){
							$Request['p_mokuteki'] .= ',';
						}
						$Request['p_mokuteki'] .= $Request['preDest'] . '-' . $Request['preCountry'] . '-' . $val;
						$cnt++;
					}
				}
				//HungLV_20150526_end
		...
		/*国内フリープランの場合または専門店国内*/
		if(($Request['MyType'] == 'freeplan-d' || $Request['MyType'] == 'senmon_kokunai') && $this->ResObj->p_hit_num > 0 && ($Request['SetParam'] != 'p_kikan_min' && $Request['SetParam'] != 'p_kikan_max')){
...
EOD;
			// ブラウザバック
			if((isset($Request['browser_back_flag']) && !empty($Request['browser_back_flag']))){
				$RetOptMax =<<<EOD
				<option value=""></option>
EOD;
			}
			...
			foreach($FacetAry as $MinMax =>$data){
				if($data['facet'] > 0){
					...
					// ブラウザバック
					if((isset($Request['p_kikan_min']) && !empty($Request['browser_back_flag']))){
...
EOD;
			...
			if((isset($Request['browser_back_flag']) && !empty($Request['browser_back_flag']))){
				...
				$RetJS =<<<EOD
	$('#{$this->MyNaigai}SearchBoxTour select#p_kikan_min').html('{$RetOpt}');
	$('#{$this->MyNaigai}SearchBoxTour select#p_kikan_max').html('{$RetOptMax}');
EOD;
			}
			else{
				//改行とタブトル
				...
				$RetJS =<<<EOD
	$('#{$this->MyNaigai}SearchBoxTour select#p_kikan_min').html('{$RetOpt}');
	$('#{$this->MyNaigai}SearchBoxTour select#p_kikan_max').html('{$RetOpt}');
EOD;
			}
...
//$('#{$this->MyNaigai}SearchBox select#pre{$TgAppend}').append('{$RetOpt}');
		$RetJS =<<<EOD
$('#{$this->MyNaigai}SearchBoxTour').find('select#pre{$TgAppend}').append('{$RetOpt}');
{$addRetJS}
EOD;
...
EOD;
			...
			for($i=1; $i<=$MyLastDay; $i++){
				...
				if($this->FacetObj->RetFacet['p_dep_day'][$MyDay]['facet'] > 0 && $MyDay>=date('Ymd') ){
					...
					$Tr .=<<<EOD
<td{$Class}><a href="#" onclick="SWDateTour(\'{$WriteMyDay}\');void(0);return false;">{$i}</a></td>
EOD;
				}
				//ファセットない
				else{
...
EOD;
			...
			//ファセットある
				...
				$MonthView =<<<EOD
<a href="#" onclick="SWDateTour(\'{$MyY}/{$MyM}\');void(0);return false;">{$MyY}年{$ViewMonth}月</a>
EOD;
			}
...
class SearchActionForFacet extends SearchActionDefault {
	...
	function __construct($naigai,$kyotenCode,$RqPara ='', $temp,$flg=''){
		...
		global $PathSharing, $GlobalSolrReqParamAry,$kyotenId,$KyotenName,$masterCsv;
...
//		$this->OtherFacetHtmlEtc($FasetdataAry);
	...
	function GetFacetRqArrayChild($keyCode) {
		...
		$joinStr='';
		if (isset($p_hatsuAry->TgDataAry[$this->MyNaigai]) && count($p_hatsuAry->TgDataAry[$this->MyNaigai]) > 0) {
    		foreach ($p_hatsuAry->TgDataAry[$this->MyNaigai] as $kyotenCode => $hatsuAry) {
    			// 孫コードと一致
    			if ($keyCode == $kyotenCode) {
    				foreach ($hatsuAry as $code => $subKyotenName) {
	...
	function contentsHeaderHtml($facetdataAry){
		...
		foreach($this->depLinkAry as $bigKyotenId => $dataAry){
			...
			foreach($dataAry as  $_kyotenId =>$data){
				...
				$disableClass = "";
				// ファセットないなら
				if(!isset($facetdataAry[$_kyotenId]) || $facetdataAry[$_kyotenId]['facet'] <= 0)
				{
