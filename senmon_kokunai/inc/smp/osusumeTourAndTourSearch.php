<div class="wr-block mb20">
    <div class="frame swiper-container" id="osusume_tour_list">

        <?php if ($kyotenId == 'index'): // botの場合は最新２７拠点表示 ?>
            ...
            ?>
        <?php elseif(isset($osusumeCsv[OSUSUME_CATEGORY_NUM][TOUR_ICHIOSHI_TOUR]) && 0 < $osusumeCsv[OSUSUME_CATEGORY_NUM][TOUR_ICHIOSHI_TOUR]): ?>
            <ul class="clearfix swiper-wrapper">
                <?php $num=0;?>
                <?php foreach($osusumeCsv[OSUSUME_COURSE] as $value):?>
                    <?php // イチオシツアー ?>
                    <?php if($value['q_group'] == TOUR_ICHIOSHI_TOUR):?>
...
<script type="text/javascript">
    // モーダルに入れる
    //if(typeof $('#other_facet').val() !== 'undefined' && 0 < $('#other_facet').val().length){
    //    var other_facet_modal = $.parseJSON($('#other_facet').val());
    //    $(".js_KinrinHatsuMenuSenmon .GlMenuCtsSenmon dl").append(other_facet_modal);
    //}
    $(".js_KinrinHatsuMenuSenmon .GlMenuCtsSenmon dl").html($('.js_HatsuMenuSenmon dl').html());
</script>
...
<?php
    ...
    new setSearch(TOUR_STRING);
 ?>

<?php // コース番号検索 ?>
<?php include_once($PathSenmonCommon.'inc/smp/courseSearch.php');?>
