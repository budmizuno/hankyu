...
//				}
		...
		if($severEnvironment == LEAFNET){
			...
			foreach ($tour_url_merge as $key => $value) {
				if(strpos($value,'search_i') !== false || strpos($value,'detail_i') !== false ){
			...
			$sub_special_tour_list = $this->soapObj->result->return->p_ab_special_response;
	...
	private function getMergeArray($special_tour_list,$newArray,$kind){
		...
		foreach((array)$special_tour_list as $key => $special_tour)
		{
			...
			if((!isset($newArray[$key]['p_hatsu_sub_name']) || strlen($newArray[$key]['p_hatsu_sub_name']) == 0) && !empty($special_tour->p_hatsu_sub_name))
			{
				$p_hatsu_sub_name = $special_tour->p_hatsu_sub_name;
