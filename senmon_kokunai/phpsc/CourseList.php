...
class CourseList{
	...
	function make($docs){
		...
		foreach($docs as $no => $paramAry){
			...
			//URL
			$this->dispTour->$no->url = $PathHttpTopN . '/tour/detail_' . $this->naigai . '.php?p_course_id=' . $paramAry->p_course_id . '&p_hei=' . $paramAry->p_hei;
			//料金ヨリマデ
			...
			foreach($paramAry as $name => $val){
				switch($name){
					...
					case 'p_transport_name':
						...
						break;
					case 'p_city_cn':
							$this->dispTour->$no->cityNameAry = $this->p_array_unique($val,$no);
						break;
					case 'p_carr_cn':
							$this->dispTour->$no->carrNameAry = $this->p_array_unique($val,$no);
						break;
					case 'p_hotel_name':
							$this->dispTour->$no->hotelNameAry = $this->p_array_unique($val,$no);
						break;
					default:
						break;
				}
	...
	#	ユニークの配列へ
	#=======================
	function p_array_unique($val,$no){
		$tmpAry = array();
		if(is_array($val)){
			foreach($val as $key => $val){
				$data = explode(',', $val);
		...
		if(is_array($destAry)){
			foreach($destAry as $countries){
				foreach($countries as $countryID => $cities){
					if(!empty($this->dispTour->$no->mokuteki)){
						$this->dispTour->$no->mokuteki .= '、';
					}
